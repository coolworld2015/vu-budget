...
export default {
	...
	created() {
		...
		appConfig.$on('searchQuery', searchQuery => {
			...
			if (searchQuery == '') {
				this.items = appConfig.projects.items.slice(0, 20);
				this.filteredItems = appConfig.projects.items;
			}
		})
	},
	methods: {
		fetchData() {
			this.$http.get(appConfig.URL + 'projects/get', {headers: {'Authorization': appConfig.access_token}})
				.then(result => {
					appConfig.projects.items = result.data.sort(this.sort);
