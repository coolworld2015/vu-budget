...
export default {
	...
	created() {
		...
		appConfig.$on('searchQuery', searchQuery => {
			...
			if (searchQuery == '') {
				this.items = appConfig.departments.items.slice(0, 20);
				this.filteredItems = appConfig.departments.items;
			}
		})
	},
	methods: {
		fetchData() {
			this.$http.get(appConfig.URL + 'departments/get', {headers: {'Authorization': appConfig.access_token}})
				.then(result => {
					appConfig.departments.items = result.data.sort(this.sort);
